...
int score = 0;
//screen kholte hi intlly kya show ho
int level = 0, size = 2;
//level se diffclty level and size se manily pad pr antar dikh rha ha

int game_level[] = {35, 25, 15};
float rate = game_level[level];
//level zayda, speed increase

GLfloat paddle_size[] = {2, 4, 6};
//width/length ko centre vale se chnage krdo

//The grid parameters for the bricks
int rows = 4;
int columns = 10;
//upper kitni bricks ha
	...
	if(start == 0)
	{
		for(i=1; i<=rows; i++)
		{
			for(j=1; j<=columns; j++)
			{
				...
				//bricks ke beech ki space badhani ha :0.6 ko increase
			}
		}
	}
	glPushMatrix();
...
void text(int sc)
{
	...
	glTranslatef(-1,0,0);
	//changing the position of text at the centre
...
void display()
{
	...
	glLoadIdentity();
	glTranslatef(0,0,-25);
	//jitni zyada neg value hogi utna zoom out hoga i.e. zyda chota chota dikhega
